...
社区常用的长提示词骨架（即梦/Dreamina 平台；field-observed）。默认只写阶段顺序；只有当前平台和操作的证据明确支持精确时间段时，才改成秒数范围。引用位置必须由当前平台配置的类型化绑定片段解析；下面不规定任何字面标签：

```
【风格】[媒介、质感、色调，一句话]
【阶段】开始：[画面+镜头+音效]；随后：[画面+镜头+音效]；最后：[画面+镜头+音效]
【声音】[对白/环境声/音效/无配乐]
