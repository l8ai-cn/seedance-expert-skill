...
V7 migration boundary: this table is a legacy discovery inventory, not an executable contract. Only operation-specific entries in `profiles/profile-index.json` may drive request transport or prompt-visible bindings, and those profiles remain disabled candidates. Fixed tokens shown by a provider are surface-local examples, never defaults for another row.
...
| Replicate | third-party model-host surface | Hosted Seedance route requiring a fresh provider check | Legacy inventory only: no retained V7 operation profile currently establishes Replicate request transport or prompt-visible binding syntax. Recheck the current provider-owned model schema, pricing, limits, and access before use; never borrow a token from another surface. The repo endorses no host; listed for completeness. |
