...
**References (r2v):** the candidate `fal.reference-to-video` profile derives compact media ordinals from one-based positions in separate arrays; current field descriptions allow up to 9 images, 3 videos, 3 audio files, and 12 files total. Images JPEG/PNG/WebP ≤30 MB; videos 480–720p, combined ≤15s, <50 MB total; audio MP3/WAV ≤15 MB each, combined ≤15s; **audio requires ≥1 image or video.** Recheck the endpoint schema and profile evidence before use.
