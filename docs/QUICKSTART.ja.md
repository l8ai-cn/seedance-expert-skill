...
スクリプトはレビュー済みの許可リストから最小ランタイムを構築・検証し、`~/.codex/skills/seedance-20`（または `$CODEX_HOME/skills/seedance-20`）へトランザクション方式でインストールします。直前の版はロールバック用に保持されます。Codex を再起動してから `$seedance-20` を呼び出してください。
...
まず `python tools/runtime_package.py --output dist/seedance-20` を実行し、生成された `dist/seedance-20` フォルダをクライアントのスキルディレクトリへコピーします。よくある置き場所は [README のインストール表](../README.md#install) にまとめてあります（保証ではありません。必ずお使いのクライアントでご確認ください）。
...
- **各参照を利用中のプラットフォーム／画面プロファイルで解決します。** 外部ハンドルはバイト単位で保持し、API の連番は検証済みプロファイルだけが生成し、構造化ロールにはトークンを追加しません。別プラットフォームの表記を推測しないでください。
