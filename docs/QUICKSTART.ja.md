...
このリポジトリを `seedance-expert` という**ひとつの**ルートスキルとして入れます。サブスキルと references は相対パスで自動的に読み込まれます。
...
リポジトリを `~/.codex/skills/seedance-expert`（または `$CODEX_HOME/skills/seedance-expert`）へコピーします。Codex を再起動してから `$seedance-expert` を呼び出してください。
...
フォルダを、名前を `seedance-expert` のままクライアントのスキルディレクトリへコピーします。よくある置き場所は [README のインストール表](../README.md#install) にまとめてあります（保証ではありません。必ずお使いのクライアントでご確認ください）。例：Claude Code `.claude/skills/`、Cursor `.cursor/skills/`、GitHub Copilot `.github/skills/`、Windsurf `.windsurf/skills/`。
