...
- Added the candidate-only V7-10 evaluation programme: seven exact offline suite bindings, six declared metamorphic relations, a complete 22-dimension observable catalog with repeatable atomic rubric items, exact ten-attempt benchmark manifests, provenance-bound original-video and derived-frame evidence, double review with adjudication, and failure-first aggregation. Every public artifact remains offline, non-release, and prohibited from supporting a model-quality claim.
- Added a dependency-free evaluation validator with strict stable-snapshot JSON/path handling, raw-byte schema/manifest links, bounded media loci, explicit request-template/settings/seed comparability, 33 closed semantic mutations detected at 100% against a 90% floor, four aggregation probes, deterministic provenance summaries, V7-09 byte locks, and ten-process cross-platform stress coverage.
- Added seven blind public development cases for expected-route leakage, baseline/style-paired operation invariance, returned-media evidence, final-frame temporal limits, complete-attempt retention, and the synthetic-fixture boundary. The public corpus now contains 145 non-release-eligible cases.
- Added `docs/V7_EVALUATION_MIGRATION.md`, documenting the separation between public text cases, deterministic/metamorphic oracles, returned-media review, and protected release evidence.
- Added the candidate-only V7-09 AV contract family: exact-version scene/policy/compile/program/render records for typed speakers, byte-exact utterances, prompt-locale/spoken-language separation, explicit editorial transitions, scoped audio continuity, post-only subtitles/dubs, and paired English/Simplified Chinese preview provenance. Provider activation and execution remain disabled.
- Added dependency-free V7-09 semantic, compiler, and AV take-review tools plus adversarial tests for utterance mutation and Unicode normalization, speaker/turn/transition reordering, authorization and policy mismatch, post-dub leakage, final-frame audio overclaim, exact-range gating, deterministic diagnostics, and ten-process stress.
- Added `docs/V7_AUDIO_MULTISHOT_MIGRATION.md` and rewrote active audio/multi-shot guidance around eight independent evidence questions: model AV capability, surface audio input, surface audio output, dialogue, lip-sync, voice reference, multi-shot grammar, and returned adherence.
- Added the candidate-only V7-08 project-state-v2 and non-destructive v1 migration layer: six closed contracts, dependency-free semantic validation and migration tools, explicit legacy mapping/disposition records, raw-byte plus canonical provenance hashes, sourced owner-scoped motion with coordinate frames, local endpoint/carry decisions, measured-drift re-anchor policy, blocked exact timing, and fail-closed `compile_required` state. Migrated source bytes remain untouched, target/dimension authority stays unresolved pending a reference manifest, and no provider profile is activated.
- Added a dependency-free V2 auxiliary checker and conditional take-review/prompt-spec contracts. Final decisions are distinct from pending confirmation; still frames cannot finally assert temporal endpoints; beat/endpoint sets are disjoint and unique; accepted-parent prompt specs bind take, review, snapshot, and media provenance; no prompt, render, provider, or submission field is permitted.
- Added 12 public development eval cases for hidden-architecture refusal, mirror-lag uncertainty, visible contact consequences, stylized world rules, local/dynamic endpoints, camera-versus-subject ownership, open-motion handoff, scoped axis reset, full-chain observability, seed uncertainty, and repeated-failure uncertainty. The public corpus now contains 138 non-release-eligible cases.
- Added a bounded mechanics-claim audit and V7-08 failure tests covering unsupported physics/world-model, hidden-capacity, training-cause, sampling, deterministic-seed, repeat-count, and universal chain-depth claims in active guidance and eval oracles. Diagnostics are deterministic and do not echo matched text.
- Added the candidate-only V7-07 paired-language compiler: a hash-bound English/Simplified Chinese realization catalog carrying an unauthenticated human-attestation declaration, one surface-independent semantic program, typed locale renderers, structural-parity hashes, and stable fail-closed diagnostics. Catalog forms, compiler grammar, and final renders still require publishing review. Exact dialogue/voiceover and multi-shot rendering remain deferred.
- Added four closed language-rendering schemas, four valid contract fixtures, two offline runtime tools, and 42 focused semantic/compiler tests covering exact catalog coverage, causal order, entity aliases, ambiguous direction, final endpoints, prompt injection, Unicode/provider-token attacks, surface-swap invariance, binding-to-unit provenance, structured first/last-frame isolation and request-carried provenance, cross-pass evidence coherence, evidence expiry, deterministic output, and 10,000 seeded safe catalog variants.
- Added the candidate-only V7-06 target/dimension reference planner and causal scene-IR preflight, including strict manifest, scene, and planning-report contracts. These checks are planning heuristics, not claims about hidden model architecture or guaranteed physics.
- Added the non-activating V7-05 model/surface profile layer: one model profile, three provider profiles covering four exact operations, five closed JSON schemas, an evidence-linked profile index, and a standard-library typed binding renderer.
- Added 30 focused binding tests, including 10,000 seeded Unicode round trips, strict JSON/control-character rejection, candidate/model/operation expiry gates, media-ordinal construction, structured first/last-frame isolation, link/hash/ancestor-swap and torn-read attacks, deterministic 10-process output, secret noninterference, four-contract schema closure, token-provenance/delimiter attacks, candidate-only schema gates, and retired-guidance closure.
- Added the non-activating v7 claim-level evidence control plane: 17 atomic claims, 6 dated source snapshots, 6 bounded retained captures, a closed authority allowlist, an exact runtime-file inventory, conflict/supersession semantics, exclusive UTC TTLs, and a closed policy pinned to exact claim bytes.
- Added an offline scheduled freshness proposal workflow with separate read-only validation and bounded write jobs. It cannot fetch sources, renew evidence, approve claims, or activate runtime guidance.
- Added 65 fail-closed evidence tests covering strict JSON, zero-fetch schema handling, transitive provenance pins, capture tamper, path/link and ancestor-swap attacks, policy deletion/downgrade, runtime drift, conflicts, expiry, deterministic reports, workflow permissions, and literal shell syntax.

### Changed

- Preserved the 147-file installable runtime and all V7-07 through V7-09 contract/tool/fixture bytes. V7-10 evaluation files are repository-only development controls; provider profiles, trusted AV policy, evidence activation, held-out execution, and release gates remain disabled.
- Removed unsupported active guidance that presented language-specific dialogue capacities, direct voice-reference lip-sync, a sound-layer priority ladder, audio as a proven internal clock, universal `Shot N` grammar, exact ranges, or seconds-per-shot budgets. Controlled tests now require a selected surface/operation policy and separately review every claimed outcome.
- Reworked surface-free audio and multi-shot evals so they reward semantic contracts, evidence resolution, and explicit uncertainty instead of unsupported timing or provider syntax; the public live canary no longer treats text-only answer wording as proof of Seedance lip-sync.
- Expanded the installable runtime to 147 files. Six files are `mapped_candidate`, 60 contain no volatile claims, 81 inherited files remain `legacy_blocked`, and provider/evidence activation remains disabled.
- Replaced global motion/finality assumptions with owner-scoped handoffs and local endpoints: a subject may settle while ambient, cyclic, exit, or camera motion continues. A still may establish visible pose and framing but not velocity, camera phase, or audio phase. Camera coverage is declared for review and never presented as proof of returned-pixel visibility or physical accuracy.
- Reframed chain depth as lineage context and optional project policy rather than a universal drift predictor; re-anchors now require a named continuity failure or an explicitly recorded local policy and reason. Exact timing ranges require current evidence for the selected surface and operation, and remain explicitly blocked in V7-08 because the installable state validator has no trusted evidence registry.
- Preserved the complete V7-07 compiler toolchain byte-for-byte. It does not consume project-state-v2 and cannot claim project-state provenance; v2 `compile_required` work fails closed until a compatible compiler exists.
- Replaced the unreleased candidate V7-06 13-dimension reference partition with a 15-dimension draft that explicitly adds `opening_state` and `endpoint_framing`; all checked-in manifests were migrated. No active 6.6 contract or enabled profile changed, and future post-release breaking changes require a schema-version bump.
- Paired English/Chinese output now renders from one validated semantic trace carrying an unauthenticated attestation declaration instead of translating a finished prompt. Provider handles remain byte-exact, surface syntax stays owned by the selected candidate profile, and a locale switch cannot alter authority, binding identity, media, or order.
- Expanded the installable runtime to 135 files. Four profile files remain `mapped_candidate`, 38 files contain no volatile claims, 93 inherited files remain `legacy_blocked`, and activation remains disabled.
- Replaced universal reference-token guidance at the routing/compiler and six-language entry points with the surface contract `semantic binding ID + media type → external opaque handle, evidence-pinned media ordinal, or structured role → typed renderer`. fal compact ordinals are derived from per-media array positions; Volc reference generation fails closed to the retained `图片{n}` image form; BytePlus remains external-handle-only. Fixed numbered handles remain only in six machine-quarantined runtime Markdown files.
- Expanded the installable runtime from the historical V7-04 baseline of 103 files to 114 files. Four profile files are `mapped_candidate`, 17 files contain no volatile claims, 93 inherited files remain `legacy_blocked`, and activation remains disabled.
- Moved the former `.github/evidence-v2` shadow records into the canonical `research/evidence/` registry, declared every evidence instance in strict schema validation, and kept the legacy runtime unchanged. The initial 103-file audit intentionally blocks release on 92 files pending the staged v7 migration.
- Excluded `research/` and `schemas/evidence-*` control files from the installable package while preserving the exact 103-file runtime tree hash.

### Fixed

- Closed evaluation-integrity gaps where a checker could otherwise omit attempts, accept reviewer/generator collisions, pass temporal claims from a still or metadata, lose disagreements during aggregation, depend on annotation row order, or promote a synthetic contract fixture into quality/release evidence.
- Removed the legacy continuity bypass where any prose containing `intentional` or `axis reset` could authorize unrelated state changes; allowances are dimension-scoped, with axis reset limited to travel direction. Legacy V1 routing now excludes V2 records and rejects ambiguous nested tracked values plus invalid lineage/index graphs.
- Closed V2 adversarial gaps for dropped open motion, discontinuous extension depth, contradictory local completion/carry state, final-frame motion overclaim, unknown or retired bindings, self-authorized provider policy, fabricated exact-timing evidence, recursive beat-graph exhaustion, schema/checker type drift, duplicate endpoint identity, atom-meaning swaps, overlapping beat scopes, silent legacy-field loss, false mapped dispositions, provider-token replacement, missing asset/take digests, raw-source ambiguity, malformed-source tracebacks, and nonterminal review state masquerading as accepted canon.
- Closed adversarial renderer tracebacks for unhashable JSON fields, extreme nesting, huge numeric tokens, non-finite exponent forms, missing roots, and unsafe Unicode format controls. Request-file reads now compare ctime/mode/link state and require two identical descriptor passes, preventing same-size torn snapshots even on coarse-timestamp filesystems.
- Aggregated model and operation evidence into one effective render gate and provenance list, so a refreshed surface claim cannot outlive stale model evidence.
- Removed misleading universal-tag teaching from Golden Prompts, standalone examples, copy-oriented catalogs, and native-reader examples; added a repository-wide active-Markdown quarantine audit and particle-safe multilingual binding clauses.
