...
**Product I2V:** After the typed product binding: `为产品参考，严格保持logo、标签、瓶身形状和颜色不变。镜头缓慢推进到标签特写；左侧暖光扫过玻璃，水珠沿瓶身下滑，背景保持暗色静止。声音：轻微环境声，结尾一声清脆玻璃音。`
...
**First/last frame:** Assign the verified structured endpoint roles, then prompt: `保持同一原创角色、服装和房间布局，角色从椅子上缓慢站起，走到窗边并停在尾帧姿势。动作连续自然，不跳切，不改变脸部、衣服或房间结构。镜头固定中景，仅轻微推镜。声音：安静室内环境声。`
...
**Camera-reference R2V:** Compile an identity binding with `锁定原创角色造型和服装` and a video binding with `仅参考运镜节奏和侧向跟拍方式，不复制人物、场景或品牌`, then append: `角色在雨夜街道从画面左侧跑入，停在路灯下回头。镜头横向稳定跟拍，最后固定中景。冷色雨光，地面反射霓虹。声音：雨声、脚步声，无配乐。`
