...
- **Surface bindings are typed and fail closed.** The V7-05 runtime separates request transport from prompt syntax, loads only byte-pinned profiles, preserves valid external opaque handles, derives media ordinals only from evidence-pinned formatters and computed request positions, and keeps structured frame roles out of prose. Unknown/disabled/stale operations, caller overrides of derived syntax, ambiguous Unicode format controls, and unstable request-file snapshots are rejected. Candidate preview cannot activate a profile or bypass either model- or operation-evidence expiry.
