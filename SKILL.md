...
8. Multilingual gate: if the prompt uses Chinese, Russian, Japanese, Korean, Spanish, or code-mixed wording, load `[ref:multilingual-community-examples]` and preserve exact opaque surface bindings byte-for-byte without translating or normalizing them. For a V7 English/Chinese pair, load `[ref:prompt-compiler]`: both locales must come from one validated semantic program and a hash-bound catalog with a human-attestation declaration. Structural parity is machine-checked. The declaration is not authenticated; translation meaning, naturalness, compiler grammar, and the final pair must be separately human-reviewed. Missing catalog entries, exact dialogue or voiceover, and multi-shot input fail closed in V7-07. For native Chinese, Japanese, or Korean example-driven requests outside that closed compiler path, route to `[skill:seedance-examples-zh]`, `[skill:seedance-examples-ja]`, or `[skill:seedance-examples-ko]`.
...
12. Quality pass: run anti-slop and the directing coherence test, then check one visible beat, one primary camera move, physically motivated light, sound intent, continuity anchors, constraints, delivery caveats, and source-date caveats. For interaction-heavy or fragile shots, verify an ordered visible chain from initial state through trigger, decisive change, response, follow-through, and settled endpoint; name the exact events the camera can observe. Treat this as a planning heuristic, never as a claim about Seedance internals or guaranteed physics.
...
- legacy project-state `tag` and single-role fields remain v6 compatibility input until V7-08; they cannot decide V7 reference authority or become provider syntax;
